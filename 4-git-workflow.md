...
### 본문 작성 — 추상화 먼저, 구체화는 그다음

PR 본문이 너무 구체적이고 장황하면 리뷰어가 "무슨 말인지" 파악하지 못한다. **기본적인 컴퓨터공학·도메인 지식이 있는 사람**이 읽고 변경의 요지를 바로 이해할 수 있게 쓴다.

- **먼저 추상화한다.** 개요는 "무엇을 왜 바꿨는지"를 몇 문장으로 요약한다 — 클래스/메서드/파라미터 이름을 나열하지 말고, 이 변경이 **어떤 문제를 풀고 어떤 동작을 바꾸는지**를 한 단계 높은 수준에서 설명한다. 리뷰어가 개요만 읽어도 diff의 목적과 큰 그림을 잡을 수 있어야 한다.
- **구체화는 그다음.** 상세 내용 섹션에서 구현 세부(핵심 파일/흐름/결정)를 적되, 요지를 이해하는 데 필요한 만큼만 적는다. 코드에서 바로 읽히는 내용을 그대로 옮겨 적지 않는다.
- 스펙 대비 의도적으로 벗어난 결정이 있으면 그 사유를 본문에 명시한다(`docs/decisions/` ADR 링크 우선).
