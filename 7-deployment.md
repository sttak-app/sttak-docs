...
> dev 환경의 실제 부트스트랩·일상 배포·롤백·트러블슈팅 절차는
> **`sttak-infra/docs/runbook-dev.md` (배포 런북)** 가 단일 출처다.
...
- Jenkins 스케줄: 데일리 `0 13-23 * * 1-5`(매시 폴링, 수집 완료 후 회차는 NOOP), 23:30 상태 리포트(최종 캔들 5일 이상 오래됐을 때만 경보 격상), 백필은 크론 없는 파라미터 빌드. 파이프라인 정의는 `jenkins/` 디렉터리 참조.
